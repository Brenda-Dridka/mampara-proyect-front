...
const EtiquetaTable54_5 = ({ etiquetas54_5, setEtiquetas54_5 }) => {
  ...
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        ...
        setEtiquetas54_5(response.data);
        setLoading(false);
      } catch (error) {
  ...
  const handleEtiquetasChange = (newState) => {
    setEtiquetas54_5(newState); // Actualizar el estado con las etiquetas
    console.log("Etiquetas54_5 posicionadas:", newState);
    guardarEtiquetas(newState);
  };

  const handleDeleteEtiqueta = (etiquetaId) => {
    // Lógica para eliminar la etiqueta con el ID proporcionado
    const updatedEtiquetas = etiquetas54_5.filter(
      (etiquetas) => etiquetas.id !== etiquetaId
    );
    setEtiquetas54_5(updatedEtiquetas);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasExt54_5, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetas54_5.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    ...
    const selected = etiquetas54_5.find(
  ...
  const handleExtrudeEtiqueta = async (etiquetaId) => {
    try {
      // Buscar la etiqueta seleccionada
      const selected2 = etiquetas54_5.find(
        (etiqueta) => etiqueta.id === etiquetaId
      );
      ...
      // Filtrar etiquetas54_5 eliminando la etiqueta
      const updatedEtiquetas54_5 = etiquetas54_5.filter(
        (item) => item.id !== etiquetaId
      );
      setEtiquetas54_5(updatedEtiquetas54_5);

      // Guardar las etiquetas actualizadas en la API
      await axios.post(apiUrlEtiquetasExt54_5, updatedEtiquetas54_5);
  ...
  return (
    <div className="position etiquetasAgregadas">
      <h6 className="text-center tittleEtiquetas">Ext 54 V</h6>
      {loading ? (
